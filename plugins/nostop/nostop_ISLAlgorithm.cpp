...
namespace NoStop
{
	...
	ISLAlgorithm::ISLAlgorithm(std::shared_ptr<DiscretizedArea> _space, Agent_plugin* _agent) 
		: m_agent(_agent), m_area(_space), m_numberOfGuards(1)
	{}

	//////////////////////////////////////////////////////////////////////////
	void ISLAlgorithm::initialize(Parsed_World* parse)
	{
		NoStop::Parsed_world* wo=reinterpret_cast<NoStop::Parsed_world*>(parse->parsed_items_from_plugins[0]);    
	...
	void ISLAlgorithm::update()
	{
		...
		bool l_agentHasToExperiments = NoStop::agentHasToExperiments(l_explorationRate);
	...
	void ISLAlgorithm::compute()
	{
		...
		double l_benefit = 0;
		std::set<std::shared_ptr<Square> > l_squares = m_agent->getVisibleSquares();
	...
	{
		std::set<SquarePtr> l_visible = m_agent->getVisibleSquares();
		SquarePtr l_currentSquare = m_area->getSquare( m_area->getCoordinate(m_agent->getPosition().getPoint2D()) );
	...
	// 	//////////////////////////////////////////////////////////////////////////
	// 	void ISLAlgorithm::(std::vector<SquarePtr> & _pos)
	// 	{
	// 		_pos.clear();
	// 		_pos.reserve(m_guards.size());
	// 		for(set<AgentPtr>::iterator it = m_guards.begin(); it != m_guards.end(); ++it)
	...
	// 
	// 	//////////////////////////////////////////////////////////////////////////
	// 	void ISLAlgorithm::getGuardsCoverage( std::vector< std::vector<Real2D> > & _areas)
	// 	{
	// 		for(set<AgentPtr>::iterator it = m_guards.begin(); it != m_guards.end(); ++it)
	// 		{
	...
	// 			{
	// 				std::vector<Real2D> l_partial = l_bound[i].approxByPoints(1);
