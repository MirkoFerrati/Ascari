...
namespace NoStop {
	...
	*/
		...
		Real2D& point)
	{
		...
		if(!isfinite(firstParam))
			return 0;
		/// Computes t2.
		secondParam = (p*n1)/sinAngle;
		if(!isfinite(secondParam))
			return 0;
		...
		Real2D& point)
	{
		...
		Real2D firstOrigin = first.origin();
		Real2D secondDir = second.direction();
		Real2D secondOrigin = second.origin();
	...
	///------------------------------------------------------
	std::vector<Real2D> Line2D::intersect(const Line2D& second) const
	{
		double firstParam, secondParam;
		Real2D point;
		vector<Real2D> points;

		if( NoStop::intersect( *this, firstParam, second, secondParam, point) )
