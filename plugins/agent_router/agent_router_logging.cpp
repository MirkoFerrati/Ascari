...
{
    INFO(bestLength_string,bestLength_tag,time, name.c_str(),best_distance);
}
bestLength agent_router_logging::parseBestLength(std::string line_to_parse)
{
    ...
    if (line_to_parse.find(bestLength_tag)==std::string::npos)
...
{
    INFO(speed_string,speed_tag,time, name.c_str(),speed,omega);
}
speed agent_router_logging::parseSpeed(std::string line_to_parse)
{
    char buffer[200];
    speed temp;
    if (line_to_parse.find(speed_tag)==std::string::npos)
...
{
    INFO(position_string,position_tag,time,name.c_str(),x,y,theta);
}
position agent_router_logging::parsePosition(std::string line_to_parse)
{
    char buffer[200];
    position temp;
    if (line_to_parse.find(position_tag)==std::string::npos)
...
{
    INFO(futureCollision_string,futureCollision_tag,time, name.c_str(),other_agent.c_str(),node_id,steps);
}
futureCollision agent_router_logging::parseFutureCollision(std::string line_to_parse)
{
    ...
    if (line_to_parse.find(futureCollision_tag)==std::string::npos)
...
{
    INFO(finalTarget_string,finalTarget_tag,time,name.c_str());
}
targetReached agent_router_logging::parseFinalTarget(std::string line_to_parse)
{
    char buffer[200];
    simulation_time time;
    if (line_to_parse.find(finalTarget_tag)==std::string::npos)
...
{   
    ...
    char buffer1[200];
    startInfo temp;
    if (line_to_parse.find(startInfo_tag)==std::string::npos)
